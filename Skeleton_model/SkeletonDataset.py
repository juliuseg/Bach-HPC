...
class SkeletonDataset(Dataset):
    def __init__(self, num_samples=100, patch_size=(32, 32, 32), transform=None):
        ...
        image_path = os.path.join(directory, "skeleton_data_ver2.npy")
        label_path = os.path.join(directory, "broken_skeleton_ver2.npy")

        self.image = np.load(image_path).astype(np.int8)[0]  # Load full 3D image
        self.label = np.load(label_path).astype(np.int8)[0]  # Load full 3D label
